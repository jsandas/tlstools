...
func TestCheckOCSPWithExpiredResponse(t *testing.T) {

	testResponse, _ := checkOCSP(expiredOCSPReponse, nil)
...
func TestGetOCSP(t *testing.T) {
	...
	ocspData, err := getOCSPResp(expiredOCSPReponse, server.URL)
	if err != nil {
		t.Errorf("OCSP response error, got: %v", err)
	}

	if len(ocspData) != len(expiredOCSPReponse) {
		t.Errorf("OCSP response incorrect, got: %d, want: %d.", len(ocspData), len(expiredOCSPReponse))
	}
}
